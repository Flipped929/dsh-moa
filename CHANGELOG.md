...
- fix（v0.4.0 实证发现）：**图片材料不再计入大上下文判定**——`bigContextChars` 此前用 `statSync().size` 统计所有材料字节数，一张 580KB 截图即误触发 ">200K 字符" 大上下文降级，导致视觉任务里 devil 被错分到无图像能力的 v4-pro（read_image 失败）。图片以图像模态进上下文，与文本长度无关，现跳过图片文件。
