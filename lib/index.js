 ...
 * - navigator（内控官）为独立异步角色（2026-09-10 裁定：deepseek-official/deepseek-flash；DeepSeek 家族现仅此一个模型；可切 kimi-k3）；
...
import { getSandboxPolicy } from './sandbox.js';

export const name = 'dsh-moa';
export const inject = ['llm', 'tools', 'sandboxPolicy'];

export const Config = z.object({
  /** 用户层角色文件目录（JSON，同名覆盖包内默认角色）。 */
  rolesDir: z.string().default('~/.dsh/moa/roles'),
  /** navigator（内控官）席位（默认 DeepSeek 家族唯一模型 deepseek-flash；可切 kimi-coding/k3）。 */
  navigatorProvider: z.string().default('deepseek-official'),
  navigatorModel: z.string().default('deepseek-flash'),
   ...
   *  架构 v3.3（2026-09-12 用户裁定——DeepSeek 家族现仅 deepseek-flash 一个模型，
   *  走 DeepSeek-V4.1-flash）：常规=GLM-5.3-flash（coding plan Pro）；高难度执行=GLM-5.3（roster）；
   *  高 stakes critic / 异构 devil / 视觉 / 异步内控 navigator 全部 = deepseek-flash（原生多模态、1M 上下文）。 */
  cheapModel: z.string().default('zai-coding-cn/glm-5.3-flash'),
  visionModel: z.string().default('deepseek-official/deepseek-flash'),
  criticModel: z.string().default('deepseek-official/deepseek-flash'),
  devilModel: z.string().default('deepseek-official/deepseek-flash'),
});
  ...
  return {
    ...
    subagentFollowup: (() => {
      ...
      return typeof svc?.followup === 'function'
        ...
        || typeof svc?.[Symbol.for('dsh.subagent.deliverPrompt')] === 'function';
    })(),
    commands: typeof ctx.get?.('commands')?.register === 'function',
    sandboxPolicy: Boolean(getSandboxPolicy(ctx)),
  };
}
...
export function apply(ctx, config) {
  ...
  // subagents 服务可能晚于本插件注册（实证：同一 profile 两次启动，一次探测到、一次未探测到）——
  // 启动期探测失败不判为"能力降级"，延迟复探后按结果播报；运行时 seats.js 仍会动态取服务并抛明确错误。
  const deferred = ['subagents', 'subagentFollowup'];
  const missing = Object.entries(caps).filter(([, v]) => !v).map(([k]) => k);
  const missingHard = missing.filter((k) => !deferred.includes(k));
  if (missingHard.length) console.warn(`[dsh-moa] 能力降级：${missingHard.join(', ')} 不可用（可能 DSH 版本契约变动）`);
  if (missing.some((k) => deferred.includes(k))) {
    ctx.effect(() => {
      const timer = setTimeout(() => {
        const late = probeCapabilities(ctx);
        if (late.subagents && late.subagentFollowup) {
          console.log('[dsh-moa] subagents 服务已就绪（启动期探测为注册竞态，非契约变动）');
        } else {
          console.warn(`[dsh-moa] 能力降级：${deferred.filter((k) => !late[k]).join(', ')} 延迟复探仍不可用——席位派活将失败（检查 profile 的 subagent 行与 DSH 版本契约）`);
        }
      }, 1500);
