 ...
 * - GLM-5.3-flash：coding plan Pro 订阅额度、快 → 常规档主力（DSH spawn 常驻，zai-coding-cn 直连，alpha.4 实证）
 * - GLM-5.3：同订阅、深推理 → review-full 的 GLM 家族深审视角（roster 自带）；高 stakes critic 不用它——
 *   2026-09-02 A/B 双跑实证 glm-5.3 三题全败于 v4-pro，按预设判据（胜率<60%）回滚 v4-pro
 * - DeepSeek 系（v4-pro / v4-flash-vision-exp）：峰谷计费、价格高 → 仅补充：
 *   高 stakes critic（A/B 实证）、大上下文 devil、executor-pro 难片、navigator 内控；批量 DeepSeek 任务排低谷/周末。
 *
 * 纪律：devil 永远跨家族（防同模型盲区）；视觉材料仅 analyst 席换 k3 多模态（其余席位保持家族多样性），
 * 并追加 vision-aux 席（DeepSeek-V4-flash-vision-exp）做 k3 的交叉核验辅助（2026-09-02 用户裁定）；
 * 高 stakes critic 升 v4-pro（A/B 实证回滚）；
...
export function describeMatrix(resolved) {
  return [
    ...
    `视觉席=${resolved.visionModel}（k3 订阅多模态）+ vision-aux 辅助（deepseek-v4-flash-vision-exp 交叉核验）`,
