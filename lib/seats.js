...
/** DSH ≥0.1.2-alpha.4：宿主派活通道符号（followup 被移除，改为 internal 符号 queuePrompt，Symbol.for 进程级稳定）。 */
const QUEUE_PROMPT = Symbol.for('dsh.subagent.queuePrompt');

/** 跨 DSH 版本给席位派任务：≤alpha.3 走 followup，≥alpha.4 走 queuePrompt 符号通道。 */
export async function deliverSeatPrompt(subagents, parentAgent, childId, packet, signal) {
  ...
  const queue = subagents?.[QUEUE_PROMPT];
  if (typeof queue === 'function') {
    return queue.call(subagents, parentAgent, childId, content, source, signal);
  }
  throw new Error('subagents 契约不可用：既无 followup（DSH ≤alpha.3）也无 queuePrompt 符号（DSH ≥alpha.4）');
}
