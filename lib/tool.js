...
import { assignSeats, isImagePath, isPeakHour, modelExpiryNote } from './scheduler.js';
...
function buildTaskPacket({ task, taskId, focus, files, cardPath, wordCap, allowLong, runtime }) {
  ...
  for (const f of files ?? []) {
    if (f.image) sections.push(`\n## 材料 ${f.path}\n（图片材料——必须用 read_image 读取此指定路径：${f.path}；禁止自选其他文件替代。若该模型无图像输入能力，在结果卡 status: failed 并注明原因）`);
    else sections.push(`\n## 材料 ${f.path}\n${f.content}`);
  }
  return redactText(sections.join('\n'));
}
...
function resolveWorkspaceRoot(ctx, agent) {
  const policy = getSandboxPolicy(ctx);
  return policy?.resolve?.({ session: agent.session })?.workspaceRoot
...
export function createMoaTool(ctx, resolved, roster, state, seatPool) {
  return defineTool({
    name: 'moa',
    description: 'Mixture-of-Agents 评审融合：只需给任务，插件按模型特性×任务类型×成本自动分派席位（架构 v3.3：常规/初稿=GLM-5.3-flash 订阅；高难度执行=GLM-5.3；高 stakes critic / 异构 devil / 视觉 / 异步内控 navigator 均为 deepseek-flash——DeepSeek 家族现只此一个模型，走 DeepSeek-V4.1-flash，原生多模态、1M 上下文；有图片材料时另追加 vision-aux 交叉核验席=kimi-k3；kimi-k3 亦可指派为 architect-k3），主模型（当前会话模型）聚合裁决。devil 仅在与主模型不同家族时才是真异构——主模型同选 DeepSeek 家族时 devil 同族，须显式 seats 指派其他家族。席位默认常驻（跨任务保留上下文），fast=true 走单发省费通道（仅 DSH provider）。要自定义再用 seats/roles 参数。成本提示：每席位是完整 agent 轮次；单文件或 <10 分钟小任务请主模型直接做。',
    parameters: {
      ...
      stakes: { type: 'string', description: "high=关键产出/高 stakes（critic 席走 criticModel 深审槽，默认 deepseek-flash）" },
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        for (const n of value.expiryNotes ?? []) lines.push(`⚠️ ${n}`);
    ...
    async execute(args, exec) {
      ...
      const seatSpecs = (Array.isArray(args.seats) && args.seats.length ? args.seats : null)
        ...
        ?? assignSeats({ mode, stakes: args.stakes, needVision }, resolved, roster);
      ...
      const runCodexSeat = async (seat, entry) => {
        ...
        if (!subagents?.getProvider?.('codex')) {
          return { seat, cardPath: join(dir, `${seat.name}.md`), card: null, settle: 'error', error: 'codex runtime 未挂载（需 profile 安装 @deepseek-ai/dsh-subagent-codex 且 patch 加 subagent-codex 行）；可换第二顺位 executor-pro（deepseek-flash）' };
        }
        const cardPath = join(dir, `${seat.name}.md`);
      ...
      const peakNote = isPeakHour() && hasDeepSeekSeat
        ? '当前高峰时段（工作日 9-12/14-18 点），本次含 DeepSeek 席位，输出按峰价计费（deepseek-flash）；非紧急任务建议排低谷/周末'
        : null;
      const expiryNotes = [...new Set(cards.map((c) => modelExpiryNote(c.model)).filter(Boolean))];
