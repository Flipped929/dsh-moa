...
function buildTaskPacket({ task, focus, files, cardPath, wordCap, allowLong, runtime }) {
  if (runtime === 'codex') {
    ...
    const sections = [
      ...
      '',
      '## 输出协议',
      `1. 意见正文 ≤${allowLong ? 2000 : wordCap} 字，直接作为最终回复返回（不写任何文件）`,
      '2. 不得调用委托/派发类能力',
    ].filter(Boolean);
...
export function createMoaTool(ctx, resolved, roster, state, seatPool) {
  return defineTool({
    ...
    parameters: {
      ...
      mode: { type: 'string', description: 'review(默认三席对抗) | research(调研) | write(写作) | dev-backend(后端开发) | dev-frontend(前端开发) | test(测试) | audit(异步内控核查)' },
    ...
    async execute(args, exec) {
      ...
      const seats = seatSpecs.map((spec) => {
        ...
        return {
          ...
          runtime: spec.runtime ?? base.runtime,
          focus: spec.focus,
        };
      });
      ...
      for (const s of seats) {
        ...
        prepared.push({ seat, entry: seat.runtime === 'codex' ? null : await seatPool.ensureSeat(parent, seat, exec.signal) });
      }
      const runCodexSeat = async (seat) => {
        const subagents = ctx.get?.('subagents');
        if (!subagents?.getProvider?.('codex')) {
          return { seat, card: null, settle: 'error', error: 'codex runtime 未挂载（需 profile 安装 @deepseek-ai/dsh-subagent-codex 且 patch 加 subagent-codex 行）；可换第二顺位 executor-pro（deepseek-v4-pro）' };
        }
        const cardPath = join(dir, `${seat.name}.md`);
        const packet = buildTaskPacket({ task: args.task, focus: seat.focus, files: files.filter((f) => f.content), cardPath, wordCap, allowLong: Boolean(args.allowLong), runtime: 'codex' });
        let result = null;
        let runError = null;
        try {
          const run = await subagents.start('codex', {
            label: `moa-codex:${seat.name}`,
            parent,
        ...
        const text = typeof result?.output === 'string' ? result.output : (Array.isArray(result?.output) ? result.output.map((b) => b?.text ?? '').join('\n') : '');
        const stopReason = result?.stopReason ?? (runError ? 'error' : 'done');
        const status = text ? 'done' : 'failed';
        if (!existsSync(cardPath)) {
          writeFileSync(cardPath, `---\nactor: ${seat.name}\ntask_id: ${taskId}\nmodel: codex-runtime\n---\n\nstatus: ${status}\n\n${text || `error: ${runError ?? 'empty output'}`}\n`, { flag: 'wx' });
        }
        return { seat, cardPath, card: existsSync(cardPath) ? readFileSync(cardPath, 'utf8') : null, settle: stopReason, error: runError ?? (text ? null : (typeof stopReason === 'string' ? stopReason : 'empty output')) };
      };

      const outcomes = await Promise.all(prepared.map(({ seat, entry }) => {
        if (seat.runtime === 'codex') return runCodexSeat(seat);
        const cardPath = join(dir, `${seat.name}.md`);
