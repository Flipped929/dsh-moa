...
export function createMoaTool(ctx, resolved, roster, state, seatPool) {
  return defineTool({
    name: 'moa',
    description: 'Mixture-of-Agents 评审融合：只需给任务，插件按模型特性×任务类型×成本自动分派席位（订阅优先：GLM-5.3-flash 常规/kimi-k3 跨家族与视觉；DeepSeek 仅补充 navigator、高 stakes critic、大上下文与难片，devil 恒跨家族），主模型（当前会话模型）聚合裁决。席位默认常驻（跨任务保留上下文），fast=true 走单发省费通道（仅 DSH provider）。要自定义再用 seats/roles 参数；stakes=high 时 critic 自动升 v4-pro（A/B 双跑实证）。成本提示：每席位是完整 agent 轮次；单文件或 <10 分钟小任务请主模型直接做。',
    parameters: {
      ...
      stakes: { type: 'string', description: "high=关键产出/高 stakes（critic 席自动升 v4-pro——A/B 双跑实证回滚）" },
    ...
    async execute(args, exec) {
      ...
      const bigContextChars = filesPre.reduce((n, p) => {
        if (isImagePath(p)) return n; // 图片以图像模态进上下文，二进制字节数不代表文本长度（实证事故：580KB 截图误触发大上下文降级，devil 被错分到无图像能力的 v4-pro）
        try {
