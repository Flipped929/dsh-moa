  ...
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        if (value.audit) {
          const a = value.audit;
          lines.push(`\n## navigator 内控（${a.ok ? '已核查' : '未通过/失败'}）\n${a.preview ? String(a.preview).slice(0, 800) : (a.error ?? '(无卡)')}`);
        }
    ...
    async execute(args, exec) {
      ...
      for (const s of seats) {
        ...
        if (seat.runtime !== 'codex') {
          try {
            entry = await seatPool.ensureSeat(parent, seat, exec.signal);
          } catch (error) {
            prepareError = errorText(error);
          }
        }
        prepared.push({ seat, entry, cardPath, prepareError });
      }
      ...
      const settled = await Promise.allSettled(prepared.map(({ seat, entry }) => {
      ...
      if (shouldAudit) {
        try {
          ...
          const r = await seatPool.taskSeat(parent, navEntry, auditPacket, navCardPath, exec.signal);
          const navCard = r.card;
          audit = {
            settle: r.settle,
            cardPath: navCardPath,
            ok: Boolean(navCard) && !validateResultCard(navCard, taskId),
            preview: navCard ? redactText(navCard).slice(0, 400) : null,
          };
        } catch (e) {
