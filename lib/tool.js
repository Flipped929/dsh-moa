...
export function createMoaTool(ctx, resolved, roster, state, seatPool) {
  return defineTool({
    ...
    parameters: {
      ...
      mode: { type: 'string', description: 'review(默认三席) | review-full(联评满编：v4+k3+GLM+claude 四家族) | research(调研) | write(写作) | dev-backend(后端开发) | dev-frontend(前端开发) | test(测试) | audit(异步内控核查)' },
    ...
    async execute(args, exec) {
      ...
      for (const s of seats) {
        ...
        if (seat.runtime === 'codex' || seat.runtime === 'claude-code') {
      ...
      const runCodexSeat = async (seat, entry) => {
        ...
        let runError = null;
        try {
          // codex CLI 续接通道：同一席位跨任务用 resume <thread_id> 保留进程与上下文（2026-08-28 实证）
          const r = seat.runtime === 'claude-code'
            ? await seatPool.taskClaudeCli(parent, seat.codexEntry ?? seat, packet, exec.signal)
            : await seatPool.taskCodexCli(parent, seat.codexEntry ?? seat, packet, exec.signal);
          text = r.text;
        } catch (e) {
        ...
        if (!existsSync(cardPath)) {
          writeFileSync(cardPath, `---\nactor: ${seat.name}\ntask_id: ${taskId}\nmodel: ${seat.runtime === 'claude-code' ? 'claude-code-runtime' : 'codex-runtime'}\nstatus: ${status}${r?.costUsd != null ? `\nreal_cost_usd: ${r.costUsd}` : ''}\n---\n\n${text || `error: ${runError ?? 'empty output'}`}\n`, { flag: 'wx' });
        }
      ...
      const settled = await Promise.allSettled(prepared.map(({ seat, entry, cardPath, packet, firstDelivered }) => {
        if (seat.runtime === 'codex' || seat.runtime === 'claude-code') return runCodexSeat(seat, entry);
